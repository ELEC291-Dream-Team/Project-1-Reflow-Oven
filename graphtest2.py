...
def waitingScreen():
    ...
    if not keyboard.is_pressed("a"):
        ...
        def run(data):
            ...
            t,y = data
            if t>-1:
                xdata_raw.append(t/10)
                ydata_raw.append(y)
                yexp = expected(t, Soak_temp, Soak_time, Reflow_temp)
                ydata2_raw.append(yexp)
                if t>xsize:
                    ax.set_xlim(t-xsize, t)
                xdata = xdata_raw[-xsize:]
                ydata = ydata_raw[-xsize:]
                ydata2 = ydata2_raw[-xsize:]
                line.set_data(xdata, ydata)
                line2.set_data(xdata, ydata2)
                if t >= HEAT_TIME_PREHEAT + Soak_time + HEAT_TIME_REFLOW + Reflow_time + COOLING_TIME + DELAY_BEFORE_CLOSING:
        ...
        def on_close_figure(event):
            ...
            time_over_reflow = 0
            for i in range(t):
                if ydata_raw[i] - ydata2_raw[i] > max_over:
                    max_over = ydata_raw[i] - ydata2_raw[i]
                    time_over = i
                if ydata2_raw[i] - ydata_raw[i] > max_under:
                    max_under = ydata2_raw[i] - ydata_raw[i]
                    time_under = i
                if ydata_raw[i] > max_temp:
                    max_temp = ydata_raw[i]
                if ydata_raw[i] > Reflow_temp:
                    time_over_reflow +=  1
                max_temp_t = i
            print("Max temp: " + str(max_temp) + " at " + str(max_temp_t/10) + " seconds")
            print("Max over: " + str(max_over) + " at " + str(time_over/10)+ " seconds")
            print("Max under: " + str(max_under) + " at " + str(time_under/10) + " seconds")
            print("Time over reflow: " + str(time_over_reflow/10) + " seconds")
            sys.exit(0)
        ...
        ax.set_ylim(0, 250)
        ax.grid()
        xdata_raw, ydata_raw, ydata2_raw = [], [], []
